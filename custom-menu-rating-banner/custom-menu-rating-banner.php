<?php
/*
Plugin Name: Rating Banner as Menu Item with Two Images
Description: Adds a customizable rating banner (two images and optional text) as a menu item in Appearance > Menus.
Version: 1.1
...
function rating_banner_settings_init() {
    ...
    register_setting('rating_banner_settings_group', 'rating_banner_second_image'); // New setting for second image
    ...
    add_settings_field(
        ...
        'Second Image URL', // New field for second image
    ...
    add_settings_field(
        'rating_banner_link',
        'Link URL (for the clickable images)',
...
function rating_banner_second_image_cb() { // Callback for second image
...
function rating_banner_add_custom_menu_item($items, $args) {
    ...
    $rating = get_option('rating_banner_text');
    $stars_image_url = get_option('rating_banner_stars_image');
    $second_image_url = get_option('rating_banner_second_image'); // Get second image URL
    $link = get_option('rating_banner_link');
    ...
    if ($stars_image_url && $link) {
        ...
        // First image: Make the image clickable by wrapping it with a link
        ...
        // Second image: Add the second image if it exists
        if ($second_image_url) {
            $banner_html .= '<a href="' . esc_url($link) . '" target="_blank"><img src="' . esc_url($second_image_url) . '" alt="Second Image" style="height: ' . esc_attr($height) . 'px; width: ' . esc_attr($width) . 'px; margin-right: 10px;" /></a>';
        }
